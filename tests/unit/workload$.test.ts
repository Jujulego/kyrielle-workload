...
import { stopwatch$, type WorkloadDuration } from '@/src/utils/stopwatch$.js';
...
beforeEach(() => {
  ...
  vi.mocked(stopwatch$).mockReturnValue(
...
describe('workload$', () => {
  it('should create a workload in ready state, with defaults applied', () => {
    ...
    expect(stopwatch$).toHaveBeenCalledWith(workload.state$);
  });
